...
ctypes.windll.kernel32.SetConsoleTitleW("Eternal Shop | .gg/eternalshop | -null#0001")

activity = discord.Activity(type=discord.ActivityType.watching, name="Eternal Shop | .gg/eternalshop")
...
@bot.event
async def on_ready():
    activity = discord.Game(name=".gg/eternalshop", type=1)
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=" .gg/eternalshop "))
    print(null)
null=fade.greenblue("""
███████╗████████╗███████╗██████╗ ███╗   ██╗ █████╗ ██╗         ███████╗██╗  ██╗ ██████╗ ██████╗
██╔════╝╚══██╔══╝██╔════╝██╔══██╗████╗  ██║██╔══██╗██║         ██╔════╝██║  ██║██╔═══██╗██╔══██╗
...
███████╗   ██║   ███████╗██║  ██║██║ ╚████║██║  ██║███████╗    ███████║██║  ██║╚██████╔╝██║ 
╚══════╝   ╚═╝   ╚══════╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝  ╚═╝╚══════╝    ╚══════╝╚═╝  ╚═╝ ╚═════╝ ╚═╝
Bot Is Online. You can start boosting!
discord.gg/eternalshop""")
    ...
    for data in all_data:
        ...
        if boost_data.status_code == 200:
            if len(boost_data.json()) != 0:
                ...
                removeToken(token)
                print(f"{Fore.GREEN}  --> {Fore.WHITE}{profile} {Fore.RED}Tok3n without n1tr0")
...
@bot.slash_command(guild_ids=[settings["guildID"]], name="paypal", description="Sends link to our paypal")
async def on_message(message):
   embedVar = discord.Embed(title="<a:ppe:1017788390546096248> PayPal", description="**tboostsss@gmail.com | F&F | No Notes | **", color=0x00f2b6)
...
@bot.slash_command(
    name="stock",
    description="Allows you to view the Bots stock."
)
async def stock(
    ...
    async def on_message(message):
        ...
        emb.description = (
            f"*You need `admin` to use this command.*"
        )
        return await ctx.respond(embed=emb)

    stock = len(open('./tokens.txt', encoding='utf-8').read().splitlines())

    emb = discord.Embed(color=discord.Color.teal())
    emb.description = (
        f"**<a:fln:1018423181104644196> Nitro Tokens In Stock - {stock}** \n **<a:boost:1017722887819501660> Server Boosts In Stock - {stock * 2}**"
    )
    return await ctx.respond(embed=emb)

    
@bot.slash_command(guild_ids=[settings["guildID"]], name="vouch",
                   description="Allows you to vouch us.")
async def vouch(ctx: discord.ApplicationContext,
                reason: discord.Option(str, "Reason for vouch", required=True)):
    emb = discord.Embed(color=discord.Color.teal())
    emb.description = (
        f"**<a:egt:1017859350204780615> Thank you for leaving us a vouch!**"
    )
    return await ctx.respond(embed=emb)
...
async def boost(ctx: discord.ApplicationContext,
    ...
    if '{"message": "Unknown Invite", "code": 10006}' in dataabotinvite:
        ...
        print(f"{Fore.MAGENTA} --> discord.gg/{INVITE} appears to be a valid server")
    ...
    return await ctx.edit(content=f"**Server Boosting Finished! Leave Us A Vouch with command /vouch**")
...
def get_invite():
    while True:
        print(f"{Fore.MAGENTA}Server invite?", end="")
...
if hwid in httpx.get("https://pastebin.com/18zK16Fi").text:
